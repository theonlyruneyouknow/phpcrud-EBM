...
if (isset($_GET['rm2_id'])) {
    if (!empty($_POST)) {
        // This part is similar to the create.php, but instead we update a record and not insert
        $rm2_id = isset($_POST['rm2_id']) ? $_POST['rm2_id'] : NULL;
        ...
        // $created = isset($_POST['created']) ? $_POST['created'] : date('Y-m-d H:i:s');
        // Update the record
        $stmt = $pdo->prepare('UPDATE companionship SET companionship_id = ?, area_id = ?, real_comp = ?, rm1_id = ?, rm2_id = ? WHERE rm2_id = ?');
        ...
        $stmt->execute([$companionship_id, $area_id, $real_comp, $rm1_id, $rm2_id, $_GET['rm2_id']]);
        // $stmt->execute([$id, $name, $email, $phone, $title, $created, $_GET['companionship_id']]);
        $msg = 'Updated Successfully!';
    }
    // Get the contact from the contacts table
    $stmt = $pdo->prepare('SELECT * FROM companionship WHERE rm2_id = ?');
    $stmt->execute([$_GET['rm2_id']]);
